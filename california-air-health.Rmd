...
The existence of spatial autocorrelations is tested via Moran's I Index for PM2.5 and preventable hospitalization rate for all causes. It turns out that all causes but "Angina" and "Perforated Appendix" are spatially autocorrelated. Strong evidence of the spatial autocorrelation of PM2.5 is also presented from the test.
...
outcome_moransI <- names(datnew)[c(8:24)][which(moransI_pval > .05)]

# pm25
pm25_moransI <- Moran.I(data.frame(datnew)[,"pm25"],
                        ca.dists.inv,na.rm = TRUE)$p.value 
