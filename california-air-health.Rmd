...
#### CAR Model results (this should go last)
...
covariate <- datnew %>% filter(year == 2015)  %>%
  dplyr::select(ID,x,y,lat,long,age60_perc,bachelor_perc,
...
#matrix
stcov = datnew %>% dplyr::select(year,ID,pm25) %>% spread(key = ID,value = pm25)
...
Limitation: lot of zeros for days above national PM2.5 standard
